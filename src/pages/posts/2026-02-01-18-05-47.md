...
昨年後半から AIコーディングエージェントを使い始め、仕様駆動開発(Spec Driven Development)も実践してきた。
現在の開発スタイルをまとめておく。
...
エージェント間はコンテキストを共有せず、SoT(Single of Truth) である OpenSpec の文書（と参考資料である plan.md）、そして実装されたコードやレビュー報告書のみで情報交換する。

- **私（人間）**
  - a のオーケストレータを対話し、「やりたい事」を入念に計画する
  - b-h の作業の様子には関知しない
...
sequenceDiagram
    actor 私
    participant Backlog
    actor CC as CC<br>(オーケストレータ)
    actor Agents as サブエージェント群<br>(b〜f)

    Note over 私,Backlog: 起票
    私->>Backlog: やりたいことを起票
    Backlog-->>私: 課題キー発行 (ABC-001)

    Note over 私,CC: 壁打ち
    私->>CC: Plan モードで「ABC-001 を実施したい」
    CC->>Backlog: ABC-001 を読み込み
    loop 壁打ち
        CC->>私: 計画を提示 / 質問
        私->>CC: 回答 / 指摘
    end
    CC->>Backlog: Plan を課題に記入

    Note over 私,Agents: OpenSpec ワークフロー
    私->>CC: 「仕様作成からテストまで行って」

    rect rgb(240, 240, 240)
        Note over CC,Agents: 以降は CC が自動でオーケストレーション
        CC->>Agents: 仕様作成 → レビュー → 修正（繰り返し）
        CC->>Agents: 実装 → コードレビュー → 修正（繰り返し）
        CC->>Agents: テスト → 修正（繰り返し）
    end

    CC->>Backlog: プルリクエスト作成
    CC-->>私: 完了報告

    Note over 私,Backlog: 最終確認
    私->>私: PR レビュー / 動作テスト
    私->>Backlog: マージ
```
-->
...
Plan モードが出力する計画には行いたいことが詳細に書かれているが、Claude Code が（派生アプリであり Claude CoWork にも代表されるように）プログラミング／ITシステム開発以外の用途でも使用されるようになることを考えると、plan.md は「汎用なフリーフォーマット」であり、システム開発に特化した何らかのフォーマットが決まっていた方が、人間にも、AI にも都合が良いと思う。
...
「やりたいことを起票」から始まっているが、その粒度を決めるのは人間（私）の能力に基づく。
たとえば、「顧客管理システムをゼロから開発する」場合、（AI と対話しながら）必要な要件を洗い出し、現在の AI に投げても大丈夫なレベルまで細分化するのは私の一存であり、AI に任せるまでには至っていない。
結局、ITシステム開発において、設計以前に行うべき事もたくさんあって、そちらが注目されていくのだろう。

### 3. AI との対話、AI の成果物のレビューを効率よく行えるツール
...
設計の時点では、ホワイトボードを前に人と人が議論していくような、AIとの壁打ちの様子がリアルタイムに視覚化されるようなツールが欲しい。
成果物のレビューでは、AIがテストしたエビデンスがあり、実行環境もあり、その成果について問えるようなツールがあると良さそう。

[Cursor](https://cursor.com/) や [Antigravity](https://antigravity.google/) がそれに近いのかも知れないけど、まだ「プログラマー寄り」な印象も受ける。
...
本稿に登場したエージェントも、基本は固有のスキルを持っており、Skill 化せずエージェントのプロンプトにそれを記述すれば良いことが多い。
